...
def pasa_bajos(signal):
    ...
    y = np.copy(signal)  #copio la señal de entrada para trabajarla
    for n in range(len(signal)): #itero por los indices del dataframe
...
def pasa_altos(signal):
    ...
    for n in range(len(signal)):
        if(n<32):
            continue
        y[n] = y[n-1] - signal[n]/32 + signal[n-16] - signal[n-17] + signal[n-32]/32 #----> difference equation found in Biomedical Signal Analysis, By Rangaraj M. Rangayyan
...
def derivativa(signal):
    ...
    for n in range(len(signal)):
        if (n<4):
            continue
        y[n] = (- signal[n-2-2] - 2*signal[n-1-2] + 2*signal[n+1-2]  + signal[n+2-2])/8
    return y
...
def ventana_movil_integradora(signal):
    ...
    for n in range(len(signal)):
      ...
      for i in range(n-media_ventana,n+media_ventana+1): # i in range of one sample for each n
        suma = suma + signal[i] # equation as described in paper
      y[n] = suma/(media_ventana+1)	# dividing by N = number of samples in integration window
...
def solve(entrada,fs, numero_signal):
    
    signal = entrada.iloc[:,1].to_numpy()
    signal_x = entrada.iloc[:,0].to_numpy()
    ...
    print('s_i:', type(signal_integrada))
    
    #marca referencial
    
    signal_pico_i = signal_integrada[1] #inicializo pico de señal total inicial, REVISAR SI LO PUEDO QUITAR
    ...
    for i in range(10, len(signal_integrada)):
        
        if signal_integrada[i]>signal_pico_i:
            signal_pico_i = signal_integrada[i] # actualizando maximo total de la señal
            
        if substraer_ruido[i] > pico_ruido_i:
            pico_ruido_i = substraer_ruido[i] #actuyalizo el maximo del ruido
    ...
    ax1.plot(signal)
    ax1.set_xticks(np.arange(0, len(signal), 100))
    ...
    #GRAFICAS
    fig , ax = plt.subplots()
    plt.vlines(op_x, ymin=min(signal)-500, ymax= max(signal)+500, color = 'r', label = 'axvline - full height')
    plt.xlabel('segundos')
    plt.ylabel('mV')
    x = np.linspace(0, len(signal), num=10)
    x_val = [int(x[i]/250) for i in range(len(x))]
    plt.xticks(x, x_val)
    plt.title('señal ' + str(numero_signal))
    plt.plot(signal)
